    ...
    export default {
        ...
        computed: {
            ...
            iconType () {
                ...
                if (this.showClose) icon = 'ios-close';//版本2.0
                // if (this.showClose) icon = 'ios-close-circle';//版本大于2.0
                return icon;
            },
            transition () {
            ...
            visualValue() {
                if (this.type === 'quarterrange') {
                    ...
                    if(Array.isArray(this.internalValue)&&this.internalValue.length > 1){
                        this.internalValue = new Date(this.internalValue[0][0])
                    }
                    return this.formatDateQuerter(this.internalValue);
                }else{
                    return this.formatDate(this.internalValue);
        ...
        methods: {
              ...
              if (Array.isArray(value) && value.length > 0 &&value[0]) {
                if (value.length === 2){
                  ...
                  if(Array.isArray(value[0])&&value[0].length > 1){
                      const start = value[0][0].split("-");
                      txt = `${start[0]}年${this.quarterchange(Number(start[1]))}`
                  }else{
                     txt = start.getFullYear()+'年'+ this.quarterchange(start.getMonth()+1);
                  }
                  return txt
                }
            ...
            formatQuarter(value){
                if (Array.isArray(value) && value.length > 0 &&value[0]) {
                    return value[0].getFullYear()+'年'+this.quarterchange(value[0].getMonth()+1)
                }
            },
            quarterchange (date) {
              if (date){
