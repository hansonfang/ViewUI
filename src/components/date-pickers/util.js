...
export const formatDateQuerter = function(date, format) {
  date = toDate(date);
  if (!date) return '';
  return dateUtil.format(date, format || 'yyyy-MM-dd');
};
...
export const DEFAULT_FORMATS = {
    ...
    monthrange: 'yyyy-MM-dd',
    quarterrange: 'yyyy-MM-dd',
...
const MONTH_FORMATTER = function(value, format, RANGE_SEPARATOR) {
  if (Array.isArray(value) && value.length === 2) {
    const start = value[0];
    const end = value[1];
    ...
    if (start && end) {
      ...
      return formatDateQuerter(start, format) + RANGE_SEPARATOR + formatDateQuerter(aend, format);
    }
  } else if (!Array.isArray(value) && value instanceof Date){
    return formatDateQuerter(value, format);
  }
  return '';
};
const QUARTER_FORMATTER = function(value, format, RANGE_SEPARATOR) {
  if (Array.isArray(value) && value.length > 0 &&  value[0]) {
    ...
    if ( value.length===2){
      ...
      if (start && end) {
        ...
        let aend =  new Date(start.getFullYear(), (start.getMonth()+3), 0);
        return formatDateQuerter(start, format) + RANGE_SEPARATOR + formatDateQuerter(aend, format);
      }
    } else if(start&& start instanceof Date){
      let aend =  new Date(start.getFullYear(), (start.getMonth()+3), 0);
      return formatDateQuerter(start, format) + RANGE_SEPARATOR + formatDateQuerter(aend, format);
    }else{
      return ''
    ...
    // let aend =  new Date(value.getFullYear(), (value.getMonth()+3), 0);
    return formatDateQuerter(value, format);
  }
  return '';
};
