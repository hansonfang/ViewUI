...
<script>
    ...
    export default {
        ...
        components: { Icon, DateTable, YearTable, MonthTable, QuarterTable, TimePicker, Confirm, datePanelLabel },
        props: {
        ...
        data(){
          ...
          return {
                ...
                rightPanelDate: this.selectionMode!='date'?new Date(leftPanelDate.getFullYear()+1, leftPanelDate.getMonth() + 1, 1):new Date(leftPanelDate.getFullYear(), leftPanelDate.getMonth() + 1, 1)
            };
        },
        computed: {
            classes(){
        ...
        watch: {
            ...
            currentView(currentView){
              if (!this.type.match(/^date/)){
                const leftMonth = this.leftPanelDate.getFullYear();
                const rightMonth = this.rightPanelDate.getFullYear();
              }
              else{
        ...
        methods: {
          ...
          setPanelDates(leftPanelDate){
            ...
            const rightPanelDate = new Date(leftPanelDate.getFullYear()+1, leftPanelDate.getMonth() + 1, 1);
            ...
            panelLabelConfig (direction) {
                ...
                const handler = type => {
                  if (this.currentView == 'quarter'){
                    const fn = type == 'month' ? this.showQuarterPicker : this.showYearPicker;
                    return () => fn(direction);
                  } else{
            ...
            changePanelDate(panel, type, increment, updateOtherPanel = true){
                ...
                if (this.splitPanels){
                  ...
                  if (!type.match(/^date/)){
                    if (panel === 'left' && this.leftPanelDate.getFullYear() >= this.rightPanelDate.getFullYear()){
                      this.changePanelDate(otherPanel, type, 1);
                    }
                    if (panel === 'right' && this.rightPanelDate.getFullYear() <= this.leftPanelDate.getFullYear()){
                      this.changePanelDate(otherPanel, type, -1);
                    }
                  } else{
          ...
          showQuarterPicker (panel) {
            ...
            handlePreSelection(panel, value){
              ...
              if (currentViewType === 'year-table') this[`${panel}PickerTable`] = this.currentView == 'quarter'?'quarter-table':'month-table';
